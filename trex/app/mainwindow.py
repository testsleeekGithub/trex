...
Developped and maintained by the TRex Project
Contributors
...
if os.name == 'nt':
    from trex.utils.windows import (set_attached_console_visible,
...
# be imported if TRex was executed from another folder than trex
#==============================================================================
try:
...
#==============================================================================
from trex.utils.qthelpers import qapplication, MENU_SEPARATOR
...
#==============================================================================
from trex.config.base import _, get_image_path, DEV, PYTEST
if not PYTEST:
...
#==============================================================================
from trex import __version__, __project_url__, __forum_url__, get_versions
from trex.config.base import (get_conf_path, get_module_data_path,
                                get_module_source_path, STDERR, DEBUG,
                                debug_print, MAC_APP_NAME, get_home_dir,
                                running_in_mac_app, get_module_path,
                                reset_config_files)
from trex.config.main import OPEN_FILES_PORT
from trex.config.utils import IMPORT_EXT, is_gtk_desktop
from trex.app.cli_options import get_options
...
from trex.utils.misc import select_port
...
# from clicking the TRex icon to showing the splash screen).
try:
    from trex.utils.environ import WinUserEnvDialog
except ImportError:
    WinUserEnvDialog = None  # analysis:ignore

from trex.utils.qthelpers import (create_action, add_actions, get_icon,
                                    add_shortcut_to_tooltip,
                                    create_module_bookmark_actions,
                                    create_program_action, DialogManager,
                                    create_python_script_action, file_uri)
from trex.config.gui import get_shortcut
from trex.otherplugins import get_trexplugins_mods
from trex.app import tour
...
#==============================================================================
# TRex's main window widgets utilities
#==============================================================================
...
def get_focus_python_shell():
    ...
    from trex.widgets.shell import PythonShellWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, options=None):
        ...
        if sys.platform == 'darwin':
            spy_path = get_module_source_path('trex')
        ...
        self.path = []
        self.project_path = []
        if osp.isfile(self.TREX_PATH):
            self.path, _x = encoding.readlines(self.TREX_PATH)
        ...
        # Preferences
        from trex.plugins.configdialog import (MainConfigPage,
                                                 ColorSchemeConfigPage)
        from trex.plugins.shortcuts import ShortcutsConfigPage
        from trex.plugins.runconfig import RunConfigPage
        from trex.plugins.maininterpreter import MainInterpreterConfigPage
        ...
        from trex.plugins.layoutdialog import (LayoutSaveDialog,
        ...
        if DEV is not None:
            title = "TRex %s (Python %s.%s)" % (__version__,
                                                  sys.version_info[0],
                                                  sys.version_info[1])
        else:
            title = "TRex (Python %s.%s)" % (sys.version_info[0],
                                               sys.version_info[1])
        if DEBUG:
    ...
    def setup(self):
        ...
        status.showMessage(_("Welcome to TRex!"), 5000)
        ...
        self.register_shortcut(prefs_action, "_", "Preferences",
                               add_sc_to_tip=True)
        trex_path_action = create_action(self,
        ...
        reset_trex_action = create_action(
            self, _("Reset TRex to factory defaults"),
            triggered=self.reset_trex)
        self.tools_menu_actions = [prefs_action, trex_path_action]
        ...
        self.tools_menu_actions += [reset_trex_action, MENU_SEPARATOR,
        ...
        from trex.plugins.console import Console
        self.console = Console(self, namespace, exitfunc=self.closing,
                            profile=self.profile,
                            multithreaded=self.multithreaded,
                            message=_("TRex Internal Console\n\n"
                                    "This console is used to report application\n"
                                    "internal errors and to inspect TRex\n"
        ...
        from trex.plugins.workingdirectory import WorkingDirectory
        ...
        if CONF.get('help', 'enable'):
            ...
            from trex.plugins.help import Help
            self.help = Help(self)
        ...
        if CONF.get('outline_explorer', 'enable'):
            ...
            from trex.plugins.outlineexplorer import OutlineExplorer
        ...
        from trex.plugins.editor import Editor
        ...
        if CONF.get('find_in_files', 'enable'):
            from trex.plugins.findinfiles import FindInFiles
        ...
        if CONF.get('explorer', 'enable'):
            ...
            from trex.plugins.explorer import Explorer
        ...
        if CONF.get('historylog', 'enable'):
            ...
            from trex.plugins.history import HistoryLog
            self.historylog = HistoryLog(self)
            self.historylog.register_plugin()

        # Online help widget
        try:    # Qt >= v4.4
            from trex.plugins.onlinehelp import OnlineHelp
        ...
        from trex.plugins.projects import Projects
        ...
        from trex.plugins.externalconsole import ExternalConsole
        ...
        # TRex documentation
        doc_path = get_module_data_path('trex', relpath="doc",
                                        attr_name='DOCPATH')
        # * Trying to find the chm doc
        trex_doc = osp.join(doc_path, "TRexdoc.chm")
        if not osp.isfile(trex_doc):
            trex_doc = osp.join(doc_path, os.pardir, "TRexdoc.chm")
        # * Trying to find the html doc
        if not osp.isfile(trex_doc):
            trex_doc = osp.join(doc_path, "index.html")
        # * Trying to find the development-version html doc
        if not osp.isfile(trex_doc):
            trex_doc = osp.join(get_module_source_path('trex'),
                                    os.pardir, 'build', 'lib', 'trex',
                                    'doc', "index.html")
        # * If we totally fail, point to our web build
        if not osp.isfile(trex_doc):
            trex_doc = 'http://pythonhosted.org/trex'
        else:
            trex_doc = file_uri(trex_doc)
        doc_action = create_action(self, _("TRex documentation"),
                                   icon=ima.icon('DialogHelpButton'),
                                   triggered=lambda:
                                   programs.start_file(trex_doc))
        self.register_shortcut(doc_action, "_",
                               "trex documentation")

        if self.help is not None:
            tut_action = create_action(self, _("TRex tutorial"),
        ...
        # Status bar widgets
        from trex.widgets.status import MemoryStatus, CPUStatus
        self.mem_status = MemoryStatus(self, status)
        self.cpu_status = CPUStatus(self, status)
        self.apply_statusbar_settings()

        # Third-party plugins
        for mod in get_trexplugins_mods():
            try:
    ...
    def post_visible_setup(self):
        ...
        # Server to maintain just one TRex instance and open files in it if
        ...
        if self.open_project:
            ...
            # Load last project if a project was active when TRex
            # was closed
    ...
    def update_window_title(self):
        """Update main trex window title based on projects."""
        ...
        if missing_deps:
            QMessageBox.critical(self, _('Error'),
                _("<b>You have missing dependencies!</b>"
                  ...
                  "<br><br>"
                  "<i>Note</i>: TRex could work without some of these "
                  "dependencies, however to have a smooth experience when "
                  "using TRex we <i>strongly</i> recommend you to install "
    ...
    def setup_layout(self, default=False):
        ...
        self.first_trex_run = False
        if hexstate is None:
            # First TRex execution:
            self.setWindowState(Qt.WindowMaximized)
            self.first_trex_run = True
            ...
            # store the initial layout as the default in trex
...
#            widget.updateGeometry()
    ...
    def quick_layout_set_menu(self):
        ...
        ql_actions = [create_action(self, _('TRex Default Layout'),
                                    triggered=lambda:
        ...
        self.ql_preferences = create_action(self, _("Layout preferences"),
                                            ...
                                            context=Qt.ApplicationShortcut)
        self.ql_reset = create_action(self, _('Reset to trex default'),
    ...
    def get_focus_widget_properties(self):
        ...
        widget = QApplication.focusWidget()
        from trex.widgets.shell import ShellBaseWidget
        from trex.widgets.editor import TextEditBaseWidget
        from trex.widgets.ipythonconsole import ControlWidget
    ...
    def remove_tmpdir(self):
        """Remove TRex temporary directory"""
    ...
    def closing(self, cancelable=False):
        ...
        if cancelable and CONF.get('main', 'prompt_on_exit'):
            reply = QMessageBox.critical(self, 'TRex',
    ...
    def about(self):
        ...
        QMessageBox.about(self,
            ...
            <br>Copyright &copy; The TRex Project Contributors
            <br>Licensed under the terms of the MIT License
            <p>Created by Pierre Raybaut.
            <br>Developed and maintained by the
            <a href="%s/blob/master/AUTHORS">TRex Project Contributors</a>.
            <br>Many thanks to all the TRex beta testers and regular users.
            ...
            <p><small>Most of the icons for the TRex 2 theme come from the Crystal
            ...
            """
            % (versions['trex'], revlink, __project_url__,
    ...
    def show_dependencies(self):
        """Show TRex's Dependencies dialog box"""
...
""" % (versions['trex'],
    ...
    def google_group(self):
        url = QUrl("http://groups.google.com/group/trexlib")
        QDesktopServices.openUrl(url)
    ...
    def open_file(self, fname, external=False):
        ...
        or open file outside TRex (if extension is not supported)
        """
    ...
    def get_trex_pythonpath(self):
        """Return TRex PYTHONPATH"""
        return self.path+self.project_path

    def add_path_to_sys_path(self):
        """Add TRex path to sys.path"""
        for path in reversed(self.get_trex_pythonpath()):
            sys.path.insert(1, path)

    def remove_path_from_sys_path(self):
        """Remove TRex path from sys.path"""
        sys_path = sys.path
        while sys_path[1] in self.get_trex_pythonpath():
            sys_path.pop(1)

    @Slot()
    def path_manager_callback(self):
        """TRex path manager"""
        from trex.widgets.pathmanager import PathManager
        self.remove_path_from_sys_path()
        ...
        dialog.exec_()
        self.add_path_to_sys_path()
        encoding.writelines(self.path, self.TREX_PATH) # Saving path
    ...
    def edit_preferences(self):
        """Edit TRex preferences"""
        from trex.plugins.configdialog import ConfigDialog
    ...
    def register_shortcut(self, qaction_or_qshortcut, context, name,
                          add_sc_to_tip=False):
        """
        Register QAction or QShortcut to TRex main application,
    ...
    # ---- Quit and restart, and reset trex defaults
    @Slot()
    def reset_trex(self):
        """
        Quit and reset TRex and then Restart application.
        """
        answer = QMessageBox.warning(self, _("Warning"),
             _("TRex will restart and reset to default settings: <br><br>"
               "Do you want to continue?"),
             QMessageBox.Yes | QMessageBox.No)
        if answer == QMessageBox.Yes:
    ...
    def restart(self, reset=False):
        """
        Quit and Restart TRex application.

        If reset True it allows to reset trex on restart.
        """
        # Get start path to use in restart script
        trex_start_directory = get_module_path('trex')
        restart_script = osp.join(trex_start_directory, 'app', 'restart.py')

        # Get any initial argument passed when trex was started
        # Note: Variables defined in bootstrap.py and trex/app/start.py
        env = os.environ.copy()
        bootstrap_args = env.pop('TREX_BOOTSTRAP_ARGS', None)
        trex_args = env.pop('TREX_ARGS')

        # Get current process and python running trex
        pid = os.getpid()
        python = sys.executable

        # Check if started with bootstrap.py
        if bootstrap_args is not None:
            trex_args = bootstrap_args
            is_bootstrap = True
        else:
            is_bootstrap = False

        # Pass variables as environment variables (str) to restarter subprocess
        env['TREX_ARGS'] = trex_args
        env['TREX_PID'] = str(pid)
        env['TREX_IS_BOOTSTRAP'] = str(is_bootstrap)
        env['TREX_RESET'] = str(reset)
        ...
        try:
            ...
            # If there is an error with subprocess, TRex should not quit and
            # the error can be inspected in the internal console
            print(error)  # trex: test-skip
            print(command)  # trex: test-skip
    ...
    def _check_updates_ready(self):
        ...
        from trex.widgets.helperwidgets import MessageCheckBox
            ...
            if update_available:
                ...
                if 'Anaconda' in sys.version or 'conda-forge' in sys.version:
                    anaconda_msg = _("<hr><b>IMPORTANT NOTE:</b> It seems "
                                     ...
                                     "update.<hr>")
                msg = _("<b>TRex %s is available!</b> <br><br>Please use "
                        "your package manager to update TRex or go to our "
                        "<a href=\"%s\">Releases</a> page to download this "
                        "new version. <br><br>If you are not sure how to "
                        "proceed to update TRex please refer to our "
    ...
    def check_updates(self, startup=False):
        """
        Check for trex updates on github releases using a QThread.
        """
        from trex.workers.updates import WorkerUpdates
...
class Spy(object):
    """
    Inspect TRex internals

    Attributes:
        app       Reference to main QApplication object
        window    Reference to trex.MainWindow widget
    """
...
def run_trex(app, options, args):
    """
    Create and show TRex's main window
...
def main():
    ...
    if options.reset_config_files:
        ...
        # Reset TRex settings to defaults
        CONF.reset_to_defaults(save=True)
        return
    elif options.optimize:
        # Optimize the whole TRex's source code directory
        import trex
        programs.run_python_script(module="compileall",
                                   args=[trex.__path__[0]], p_args=['-O'])
        return
    ...
    if CONF.get('main', 'crash', False) and not DEV:
        ...
        if SPLASH is not None:
            SPLASH.hide()
        QMessageBox.information(None, "TRex",
            "TRex crashed during last session.<br><br>"
            "If TRex does not start at all and <u>before submitting a "
            "bug report</u>, please try to reset settings to defaults by "
            "running TRex with the command line option '--reset':<br>"
            "<span style=\'color: #555555\'><b>python trex --reset"
            "</b></span><br><br>"
            "<span style=\'color: #ff5555\'><b>Warning:</b></span> "
            "this command will remove all your TRex configuration files "
            "located in '%s').<br><br>"
    ...
    mainwindow = None
    try:
        mainwindow = run_trex(app, options, args)
    except FontError as fontError:
        QMessageBox.information(None, "TRex",
                "TRex was unable to load the <i>TRex 3</i> "
                "icon theme. That's why it's going to fallback to the "
                "theme used in TRex 2.<br><br>"
                "For that, please close this window and start TRex again.")
        CONF.set('main', 'icon_theme', 'trex 2')
    except BaseException:
        CONF.set('main', 'crash', True)
        import traceback
        traceback.print_exc(file=STDERR)
        traceback.print_exc(file=open('trex_crash.log', 'w'))
