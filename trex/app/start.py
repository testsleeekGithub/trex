...
from trex.utils.external import lockfile
from trex.py3compat import is_unicode


def send_args_to_trex(args):
    """
    Simple socket client used to send the args passed to the TRex
...
def main():
    ...
    TRex is already running, this will just parse and send command line
    ...
    os.environ['TREX_ARGS'] = str(sys.argv[1:])
    ...
    if CONF.get('main', 'single_instance') and not options.new_instance \
      and not options.reset_config_files and not running_in_mac_app():
        ...
        # Lock file creation
        lock_file = get_conf_path('trex.lock')
        lock = lockfile.FilesystemLock(lock_file)

        # Try to lock trex.lock. If it's *possible* to do it, then
        ...
        try:
            ...
            # module, try to remove a possibly stale trex.lock.
            ...
            # Then start TRex as usual and *don't* continue
            # executing this script because it doesn't make
            # sense
            from trex.app import mainwindow
            mainwindow.main()
            return
        ...
        if lock_created:
            ...
            # Pass args to TRex or print an informative
            # message
            if args:
                send_args_to_trex(args)
            else:
                print("TRex is already running. If you want to open a new \n"
