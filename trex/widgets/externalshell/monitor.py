...
# Local imports
from trex.config.base import get_conf_path, DEBUG
from trex.py3compat import getcwd, is_text_string, pickle, _thread
from trex.utils.misc import fix_reference_name
from trex.utils.debug import log_last_error
from trex.utils.dochelpers import (getargtxt, getdoc, getsource,
                                     getobjdir, isdefined)
from trex.utils.bsdsocket import (communicate, read_packet, write_packet,
                                    PACKET_NOT_RECEIVED, PICKLE_HIGHEST_PROTOCOL)
from trex.utils.introspection.module_completion import module_completion
from trex.widgets.variableexplorer.utils import (get_remote_data,
                                                   make_remote_view)
...
class Monitor(threading.Thread):
    ...
    def __init__(self, host, introspection_port, notification_port,
        ...
        self._mlocals = {
                       ...
                       "set_trex_breakpoints": self.set_trex_breakpoints,
    ...
    def set_trex_breakpoints(self):
        """Set all TRex breakpoints in active pdb session"""
        if not self.pdb_obj:
            return
        self.pdb_obj.set_trex_breakpoints()
