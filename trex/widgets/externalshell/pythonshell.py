...
# Local imports
from trex.config.base import (_, DEBUG, get_module_source_path,
                                MAC_APP_NAME, running_in_mac_app)
from trex.py3compat import (is_text_string, to_binary_string,
                              to_text_string)
from trex.utils import icon_manager as ima
from trex.utils.bsdsocket import communicate, write_packet
from trex.utils.environ import RemoteEnvDialog
from trex.utils.misc import add_pathlist_to_PYTHONPATH, get_python_executable
from trex.utils.programs import get_python_args
from trex.utils.qthelpers import (add_actions, create_action,
                                    create_toolbutton, DialogManager)
from trex.widgets.externalshell.baseshell import ExternalShellBase
from trex.widgets.shell import PythonShellWidget
from trex.widgets.variableexplorer.namespacebrowser import NamespaceBrowser
...
class ExtPythonShellWidget(PythonShellWidget):
    ...
    def __init__(self, parent, history_filename, profile=False):
        initial_message = _("NOTE: The Python console is going to "
                            "be REMOVED in TRex 3.2. Please start "
    ...
    def set_trex_breakpoints(self):
        """Set TRex breakpoints into debugging session"""
        return self.ask_monitor("set_trex_breakpoints()")
...
class ExternalPythonShell(ExternalShellBase):
    ...
    def create_process(self):
        ...
        if self.post_mortem_action.isChecked():
            env.append('TREX_EXCEPTHOOK=True')
        ...
        if self.monitor_enabled:
            env.append('TREX_SHELL_ID=%s' % id(self))
            env.append('TREX_AR_TIMEOUT=%d' % self.autorefresh_timeout)
            env.append('TREX_AR_STATE=%r' % self.autorefresh_state)
            from trex.widgets.externalshell import introspection
            ...
            if self.namespacebrowser is not None:
                self.configure_namespacebrowser()
            env.append('TREX_I_PORT=%d' % introspection_server.port)
            env.append('TREX_N_PORT=%d' % notification_server.port)
