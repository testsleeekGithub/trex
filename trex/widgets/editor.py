...
# Local imports
from trex.config.base import _, DEBUG, STDERR, STDOUT
from trex.config.gui import config_shortcut
from trex.config.utils import (get_edit_filetypes, get_edit_filters,
                                 get_filter)
from trex.py3compat import qbytearray_to_str, to_text_string, u
from trex.utils import icon_manager as ima
from trex.utils import (codeanalysis, encoding, sourcecode,
                          syntaxhighlighters)
...
from trex.widgets.fileswitcher import FileSwitcher
from trex.widgets.findreplace import FindReplace
from trex.widgets.sourcecode import codeeditor
from trex.widgets.sourcecode.base import TextEditBaseWidget  # analysis:ignore
from trex.widgets.sourcecode.codeeditor import Printer       # analysis:ignore
from trex.widgets.sourcecode.codeeditor import get_file_language
from trex.widgets.status import (CursorPositionStatus, EncodingStatus,
                                   EOLStatus, ReadWriteStatus)
from trex.widgets.tabs import BaseTabs
...
#            btn.setEnabled(count > 1)
    ...
    def __check_file_status(self, index):
        """Check if file has been changed in any way outside TRex:
        1. removed, moved or renamed outside TRex
        2. modified outside TRex"""
               ...
               != to_text_string(finfo.lastmodified.toString()):
                if finfo.editor.document().isModified():
                    answer = QMessageBox.question(self,
                                self.title,
                                _("<b>%s</b> has been modified outside TRex."
           ...
           and sourcecode.has_mixed_eol_chars(text):
            ...
            QMessageBox.warning(self, self.title,
                                _("<b>%s</b> contains mixed end-of-line "
                                  "characters.<br>TRex will fix this "
                                  "automatically.") % name,
                                QMessageBox.Ok)
...
class EditorMainWindow(QMainWindow):
    def __init__(self, plugin, menu_actions, toolbar_list, menu_list,
        ...
        self.setWindowTitle("TRex - %s" % plugin.windowTitle())
...
class EditorPluginExample(QSplitter):
    ...
    def register_editorstack(self, editorstack):
        ...
        editorstack.set_default_font(font, color_scheme='TRex')
...
def test():
    ...
    from trex.config.base import get_module_path
    from trex.utils.introspection.manager import IntrospectionManager

    cur_dir = osp.join(get_module_path('trex'), 'widgets')
    ...
    print("Elapsed time: %.3f s" % (time.time()-t0))  # trex: test-skip
