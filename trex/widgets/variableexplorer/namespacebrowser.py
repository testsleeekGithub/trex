...
# Local imports
from trex.config.base import _, get_supported_types
from trex.py3compat import is_text_string, getcwd, to_text_string
from trex.utils import encoding
from trex.utils import icon_manager as ima
from trex.utils.iofuncs import iofunctions
...
from trex.utils.qthelpers import (add_actions, create_action,
                                    create_toolbutton)
from trex.widgets.externalshell.monitor import (
...
from trex.widgets.variableexplorer.collectionseditor import (
    RemoteCollectionsEditorTableView)
from trex.widgets.variableexplorer.importwizard import ImportWizard
from trex.widgets.variableexplorer.utils import REMOTE_SETTINGS
...
class NamespaceBrowser(QWidget):
    ...
    def visibility_changed(self, enable):
        ...
        # This is slowing down TRex a lot if too much data is present in
        ...
        # give focus to TRex after using another application (like Chrome
        # or Firefox).
    ...
    def plot(self, name, funcname):
        if self.is_ipyclient:
            ...
            command = "import trex.pyplot; "\
                  "__fig__ = trex.pyplot.figure(); "\
                  "__items__ = getattr(trex.pyplot, '%s')(%s); "\
                  "trex.pyplot.show(); "\
    ...
    def imshow(self, name):
        if self.is_ipyclient:
            ...
            command = "import trex.pyplot; " \
                  "__fig__ = trex.pyplot.figure(); " \
                  "__items__ = trex.pyplot.imshow(%s); " \
                  "trex.pyplot.show(); del __fig__, __items__;" % name
            self.shellwidget.send_to_process(command)
