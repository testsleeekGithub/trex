...
"""Module checking TRex installation requirements"""
...
def show_warning(message):
    ...
    try:
        ...
        root = Tkinter.Tk()
        root.withdraw()
        tkMessageBox.showerror("TRex", message)
    except ImportError:
        pass
    raise RuntimeError(message)

def check_path():
    """Check sys.path: is TRex properly installed?"""
    dirname = osp.abspath(osp.join(osp.dirname(__file__), osp.pardir))
    if dirname not in sys.path:
        show_warning("TRex must be installed properly "
...
def check_qt():
    ...
    try:
        ...
        if LooseVersion(actual_ver) < LooseVersion(required_ver):
            show_warning("Please check TRex installation requirements:\n"
                         "%s %s+ is required (found v%s)."
                         % (package_name, required_ver, actual_ver))
    except ImportError:
        show_warning("Failed to import qtpy.\n"
                     "Please check TRex installation requirements:\n\n"
                     "qtpy 1.1.0+ and either\n"
                     "%s %s+ or\n"
                     "%s %s+\n\n"
                     "are required to run TRex."
