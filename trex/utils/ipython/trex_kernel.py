...
if not IS_EXT_INTERPRETER:
    ...
    from trex.utils.dochelpers import isdefined, getdoc, getsource
    ...
    # We add "trex" to sys.path for external interpreters, so this works!
...
class TRexKernel(IPythonKernel):
    """TRex kernel for Jupyter"""

    def __init__(self, *args, **kwargs):
        super(TRexKernel, self).__init__(*args, **kwargs)
    ...
    def _set_trex_breakpoints(self):
        """Set all TRex breakpoints in an active pdb session"""
        if not self._pdb_obj:
            return
        self._pdb_obj.set_trex_breakpoints()
