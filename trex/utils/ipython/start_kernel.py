...
def varexp(line):
    """
    TRex's variable explorer magic
    ...
    __items__ = getattr(trex.pyplot, funcname[2:])(ip.user_ns[name])
    trex.pyplot.show()
...
def main():
    ...
    if not IS_EXT_INTERPRETER:
        from trex.utils.ipython.trex_kernel import TRexKernel
    else:
        # We add "trex" to sys.path for external interpreters,
        # so this works!
        # See create_kernel_spec of plugins/ipythonconsole
        from utils.ipython.trex_kernel import TRexKernel

    kernel = IPKernelApp.instance()
    kernel.kernel_class = TRexKernel
    try:
