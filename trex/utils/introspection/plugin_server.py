        ...
        while 1:
            ...
            if events == 0 and initialized:
                if timed_out:
                    ...
                    print('Timed out after %s sec' % delta)  # trex: test-skip
                    return
                timed_out = True
                continue
            timed_out = False
            ...
            while 1:
                ...
                if request['func_name'] == 'server_quit':
                    print('Quitting')  # trex: test-skip
                    sys.stdout.flush()
                    return
                elif request['func_name'] != 'server_heartbeat':
                    requests.append(request)
                else:
                    print('Got heartbeat')  # trex: test-skip
                try:
...
class PluginServer(AsyncServer):
    ...
    def initialize(self, plugin_name):
        ...
        mod = __import__('trex.utils.introspection.' + mod_name,
                         fromlist=[mod_name])
...
if __name__ == '__main__':
    args = sys.argv[1:]
    if not len(args) == 2:
        print('Usage: plugin_server.py client_port plugin_name')  # trex: test-skip
        sys.exit(0)
    plugin = PluginServer(*args)
    print('Started')  # trex: test-skip
    plugin.run()
