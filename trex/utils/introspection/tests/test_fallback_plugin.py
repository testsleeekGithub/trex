...
# Local imports
from trex.utils.introspection.fallback_plugin import (FallbackPlugin,
                                                        python_like_exts,
                                                        all_editable_exts,
                                                        get_parent_until,
...
def test_extensions():
    ...
    assert path == 'trex.utils.introspection.fallback_plugin'

    line = 'from trex.widgets.sourcecode.codeeditor import CodeEditor'
    ...
    path = osp.expanduser(r'~/.trex2/temp.py')
    if os.path.exists(path):
        path = get_parent_until(path)
        assert path == '.trex2.temp', path
