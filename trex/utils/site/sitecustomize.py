...
# Important Note:
#
# We avoid importing trex here, so we are handling Python 3 compatiblity
# by hand.
#==============================================================================
...
#==============================================================================
# Prepending this trex package's path to sys.path to be sure
# that another version of trex won't be imported instead:
#==============================================================================
trex_path = osp.dirname(__file__)
while not osp.isdir(osp.join(trex_path, 'trex')):
    trex_path = osp.abspath(osp.join(trex_path, os.pardir))
if not trex_path.startswith(sys.prefix):
    # TRex is not installed: moving its parent directory to the top of
    # sys.path to be sure that this trex package will be imported in
    # the remote process (instead of another installed version of TRex)
    while trex_path in sys.path:
        sys.path.remove(trex_path)
    sys.path.insert(0, trex_path)
os.environ['TREX_PARENT_DIR'] = trex_path
...
if sys.platform == 'darwin':
    ...
    if MAC_APP_NAME in __file__:
        if IS_EXT_INTERPRETER:
            # Add a minimal library (with trex) at the end of sys.path to
...
#==============================================================================
if IS_IPYKERNEL or os.environ.get('TREX_SHELL_ID') is None:
    monitor = None
else:
    from trex.widgets.externalshell.monitor import Monitor
    monitor = Monitor("127.0.0.1",
                      int(os.environ['TREX_I_PORT']),
                      int(os.environ['TREX_N_PORT']),
                      os.environ['TREX_SHELL_ID'],
                      float(os.environ['TREX_AR_TIMEOUT']),
                      os.environ["TREX_AR_STATE"].lower() == "true")
    monitor.start()

    def open_in_trex(source, lineno=1):
        """
        Open a source file in TRex's editor (it could be a filename or a
    ...
    builtins.open_in_trex = open_in_trex
    ...
    if os.name == 'nt' and matplotlib and not IS_IPYKERNEL:
        ...
        def qt_nt_inputhook():
            ...
            # (otherwise, on slow machines, this may freeze TRex)
            ...
            if app and app.thread() is QtCore.QThread.currentThread():
                ...
                #socket.connectToServer(os.environ['TREX_SHELL_ID'])
...
if matplotlib is not None:
    if not IS_IPYKERNEL:
        ...
        from trex.utils import inputhooks
        if mpl_backend:
            import ctypes
...
#     I'm kind of reluctant to import trex in sitecustomize, even if this
...
def interaction(self, frame, traceback):
    ...
    if self.send_initial_notification:
        self.notify_trex(frame) #-----TRex-specific-----------------------
...
def reset(self):
    ...
    self.set_trex_breakpoints()


#XXX: notify trex on any pdb command (is that good or too lazy? i.e. is more
...
#
# NOTE: do not remove trex_path from sys.path because if TRex has been
# installed using python setup.py install, then this could remove the
# 'site-packages' directory from sys.path!
#==============================================================================
try:
    sys.path.remove(osp.join(trex_path, "trex", "widgets",
                             "externalshell"))
except ValueError:
    pass
...
class UserModuleReloader(object):
    ...
    def __init__(self, namelist=None, pathlist=None):
        if namelist is None:
            namelist = []
        spy_modules = ['sitecustomize', 'trex', 'trexplugins']
...
#==============================================================================
# Handle Post Mortem Debugging and Traceback Linkage to TRex
#==============================================================================
...
if "TREX_EXCEPTHOOK" in os.environ:
    set_post_mortem()
