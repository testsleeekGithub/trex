...
# Local imports
from trex.config.base import get_image_path, running_in_mac_app
from trex.config.gui import get_shortcut
from trex.utils import programs
from trex.utils import icon_manager as ima
from trex.utils.icon_manager import get_icon, get_std_icon
...
def qapplication(translate=True, test_time=3):
    ...
    if running_in_mac_app():
        TRexApplication = MacApplication
    else:
        TRexApplication = QApplication
    
    app = TRexApplication.instance()
    if app is None:
        # Set Application name for Gnome 3
        # https://groups.google.com/forum/#!topic/pyside/24qxvwfrRDs
        app = TRexApplication(['TRex'])

        # Set application name for KDE (See issue 2207)
        app.setApplicationName('TRex')
    if translate:
...
def add_actions(target, actions, insert_before=None):
    ...
    for action in actions:
        if (action is None) and (previous_action is not None):
            ...
            if isinstance(action, TRexAction):
...
class TRexAction(QAction):
    """TRex QAction class wrapper to handle cross platform patches."""

    def __init__(self, *args, **kwargs):
        """TRex QAction class wrapper to handle cross platform patches."""
        super(TRexAction, self).__init__(*args, **kwargs)
    ...
    def __getattribute__(self, name):
        ...
        attr = super(TRexAction, self).__getattribute__(name)
