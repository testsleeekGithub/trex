...
Changes by the TRex Team to the original module:
  * Rewrite kill Windows function to make it more reliable.
  * Detect if the process that owns the lock is an TRex one.
"""
            ...
            try:
                ...
                if e.errno == errno.EEXIST:
                    ...
                    try:
                        ...
                        if os.name == 'nt':
                            conditions = ['trex' in c.lower()
                                          for c in p.cmdline()]
                        else:
                            conditions = [p.name() == 'trex',
                                          p.name() == 'trex3']
                        # For DEV
