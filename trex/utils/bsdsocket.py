...
# and failure to read out buffers will most likely lock up TRex.
...
# Local imports
from trex.config.base import DEBUG, STDERR
DEBUG_EDITOR = DEBUG >= 3
from trex.py3compat import pickle
...
def read_packet(sock, timeout=None):
    ...
    if data is not None:
        try:
            ...
            # Catch all exceptions to avoid locking trex
            if DEBUG_EDITOR:
...
if __name__ == '__main__':
    if not os.name == 'nt':
        ...
        print("-- Testing standard Python socket interface --")  # trex: test-skip
        ...
        print('..got "%s" from %s' % (accsock.recv(4096), addr))  # trex: test-skip
        ...
        print("-- Testing BSD socket write_packet/read_packet --")  # trex: test-skip
    
        write_packet(client, "a tiny piece of data")
        print('..got "%s" from read_packet()' % (read_packet(accsock)))  # trex: test-skip
        
        client.close()
        server.close()
        
        print("-- Done.")  # trex: test-skip
