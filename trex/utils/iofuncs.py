...
# - If pandas fails to import here (for any reason), TRex
#   will crash at startup (e.g. see Issue 2300)
# - This also prevents TRex to start IPython kernels
#   (see Issue 2456)
try:
    import pandas as pd
except:
...
class MatlabStruct(dict):
    ...
    Examples
    ========
    >>> from trex.utils.iofuncs import MatlabStruct
    >>> a = MatlabStruct()
    >>> a.b = 'spam'  # a["b"] == 'spam'
...
class IOFunctions(object):
    ...
    def get_3rd_party_funcs(self):
        ...
        for mod in get_trexplugins_mods(io=True):
            try:
...
if __name__ == "__main__":
    ...
    print(" Data saved in %.3f seconds" % (time.time()-t0))  # trex: test-skip
    t0 = time.time()
    example2, ok = load_dictionary("test.spydata")

    print("Data loaded in %.3f seconds" % (time.time()-t0))  # trex: test-skip
