...
from trex.config.base import _, SCIENTIFIC_STARTUP
from trex.config.main import CONF
from trex.utils import encoding, programs, sourcecode
...
class ExternalConsole(TRexPluginWidget):
    """
    Console widget
    """
    ...
    def set_trex_breakpoints(self):
        """Set all TRex breakpoints into all shells"""
        for shellwidget in self.shellwidgets:
            shellwidget.shell.set_trex_breakpoints()
    ...
    def start(self, fname, wdir=None, args='', interact=False, debug=False,
        ...
        if python:
            if self.main.editor is not None:
                shellwidget.open_file.connect(self.open_file_in_trex)
            if fname is None:
                self.python_count += 1
    ...
    def open_file_in_trex(self, fname, lineno):
        """Open file in TRex's editor from remote process"""
