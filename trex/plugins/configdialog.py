...
# Local imports
from trex.config.base import (_, LANGUAGE_CODES, load_lang_conf,
                                running_in_mac_app, save_lang_conf)
from trex.config.gui import get_font, set_font
from trex.config.main import CONF
from trex.config.user import NoDefault
from trex.config.utils import is_gtk_desktop
from trex.py3compat import to_text_string, is_text_string, getcwd
from trex.utils import icon_manager as ima
from trex.utils import syntaxhighlighters
from trex.widgets.colors import ColorLayout
from trex.widgets.sourcecode.codeeditor import CodeEditor
...
class ConfigDialog(QDialog):
    """TRex configuration ('Preferences') dialog box"""
    ...
    def __init__(self, parent=None):
        ...
        # Signals and slots
        if self.main:
            self.button_reset.clicked.connect(self.main.reset_trex)
        ...
        # Ensures that the config is present on trex first run
...
class TRexConfigPage(ConfigPage, ConfigAccessMixin):
...
class PluginConfigPage(TRexConfigPage):
    """Plugin configuration dialog box page widget"""
    def __init__(self, plugin, parent):
        ...
        TRexConfigPage.__init__(self, parent)
...
class GeneralConfigPage(TRexConfigPage):
    """Config page that maintains reference to main TRex window
    ...
    def prompt_restart_required(self):
        ...
        if len(options) == 1:
            msg_start = _("TRex needs to restart to change the following "
                          "setting:")
        else:
            msg_start = _("TRex needs to restart to change the following "
                          "settings:")
...
class MainConfigPage(GeneralConfigPage):
    ...
    def get_font(self, option):
        """Return global font used in TRex."""
        return get_font(option=option)

    def set_font(self, font, option):
        """Set global font used in TRex."""
...
class ColorSchemeConfigPage(GeneralConfigPage):
    ...
    def setup_page(self):
        ...
        about_label = QLabel(_("Here you can select the color scheme used in "
                               "the Editor and all other TRex plugins.<br><br>"
                               "You can also edit the color schemes provided "
                               "by TRex or create your own ones by using "
    ...
    def delete_scheme(self):
        ...
        answer = QMessageBox.warning(self, _("Warning"),
                                           ...
                                           QMessageBox.Yes | QMessageBox.No)
        if answer == QMessageBox.Yes:
            # Put the combobox in TRex by default, when deleting a scheme
            names = self.get_option('names')
            self.set_scheme('trex')
            self.schemes_combobox.setCurrentIndex(names.index('trex'))
            self.set_option('selected', 'trex')
