...
# Local imports
from trex.config.base import _, get_conf_path, PYTEST
from trex.config.main import (CONF, RUN_CELL_SHORTCUT,
...
from trex.utils.qthelpers import create_action, add_actions, MENU_SEPARATOR
from trex.widgets.findreplace import FindReplace
from trex.widgets.editor import (EditorMainWindow, EditorSplitter,
                                   EditorStack, Printer)
...
from trex.plugins.runconfig import (ALWAYS_OPEN_FIRST_RUN_OPTION,
...
class EditorConfigPage(PluginConfigPage):
    ...
    def setup_page(self):
        ...
        eol_label = QLabel(_("When opening a text file containing "
                             ...
                             "on Windows platforms), TRex may fix the "
                             "file automatically."))
...
class Editor(TRexPluginWidget):
    ...
    DISABLE_ACTIONS_WHEN_HIDDEN = False # TRexPluginWidget class attribute
    ...
    def __init__(self, parent, ignore_last_opened_files=False):
        ...
        self.edit_menu_actions = None #XXX: find another way to notify TRex
        # (see trex.py: 'update_edit_menu' method)
        ...
        self.introspector = IntrospectionManager(
                extra_path=self.main.get_trex_pythonpath())
    ...
    def visibility_changed(self, enable):
        ...
        TRexPluginWidget.visibility_changed(self, enable)
...
#        self.set_option('filenames', filenames)
    ...
    def set_path(self):
        for finfo in self.editorstacks[0].data:
            finfo.path = self.main.get_trex_pythonpath()
        if self.introspector:
            self.introspector.change_extra_path(
                    self.main.get_trex_pythonpath())
        ...
        if editorstack.save():
            ...
            if runconf is None:
                ...
                if CONF.get('run', 'open_at_least_once', not PYTEST):
                    ...
                    # a script is ever run in TRex, so that the user may
            ...
            if not interact and not debug:
                ...
                # (see TRexPluginWidget.visibility_changed method)
                editor.setFocus()
