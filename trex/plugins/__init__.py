...
Here, 'plugins' are widgets designed specifically for TRex
These plugins inherit the following classes
(TRexPluginMixin & TRexPluginWidget)
"""
...
from trex.config.gui import get_color_scheme, get_font
from trex.config.main import CONF
from trex.config.user import NoDefault
from trex.py3compat import configparser, is_text_string
from trex.utils import icon_manager as ima
from trex.utils.qthelpers import create_action, toggle_actions
...
class TRexDockWidget(QDockWidget):
    """Subclass to override needed methods"""
    plugin_closed = Signal()

    def __init__(self, title, parent):
        super(TRexDockWidget, self).__init__(title, parent)
...
class TRexPluginMixin(object):
    """
    Useful methods to bind widgets to the main window
    See TRexPluginWidget class for required widget interface
    ...
    def __init__(self, main=None, **kwds):
        ...
        super(TRexPluginMixin, self).__init__(**kwds)
    ...
    def on_first_registration(self):
        ...
        # Was written to handle the very first plugin position in TRex's
    ...
    def initialize_plugin_in_mainwindow_layout(self):
        ...
        initialize plugin position in TRex's window layout"""
...
##         # are not painted after restarting TRex and restoring their hexstate)
...
##         flags = Qt.Widget if is_old_pyqt or os.name != 'nt' else Qt.Window
        dock = TRexDockWidget(self.get_plugin_title(), self.main)#, flags)
    ...
    def get_plugin_font(self, rich_text=False):
        ...
        All plugins in TRex use a global font. This is a convenience method
    ...
    def set_plugin_font(self):
        ...
        Note: All plugins in TRex use a global font. To define a different
...
class TRexPluginWidget(QWidget, TRexPluginMixin):
    """
    TRex base widget class
    TRex's widgets either inherit this class or reimplement its interface
    """
    ...
    if PYQT5:
        def __init__(self, parent, **kwds):
            super(TRexPluginWidget, self).__init__(parent, **kwds)
    else:
    ...
    def get_plugin_icon(self):
        ...
        Note: this is required for plugins creating a main window
              (see TRexPluginMixin.create_mainwindow)
