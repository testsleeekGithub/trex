...
from trex.config.base import (_, DEV, get_home_dir, get_module_path,
...
from trex.utils.misc import (add_pathlist_to_PYTHONPATH, get_error_match,
                               get_python_executable, remove_backslashes)
from trex.widgets.findreplace import FindReplace
from trex.widgets.ipythonconsole import ClientWidget
...
# Plugin widget
#------------------------------------------------------------------------------
class IPythonConsole(TRexPluginWidget):
    """
    ...
    def register_plugin(self):
        ...
        self.editor.breakpoints_saved.connect(self.set_trex_breakpoints)
        ...
        if not CONF.get('main_interpreter', 'default'):
            ...
            if not ipykernel_present:
                client.show_kernel_error(_("Your Python environment or "
                                     ...
                                     "not possible for TRex to create a "
    ...
    def set_editor(self):
        ...
        if DEV:
            ...
            bootstrap_script = osp.join(osp.dirname(trex_start_directory),
                                        'bootstrap.py')
            editor = u'{0} {1} --'.format(python, bootstrap_script)
        else:
            import1 = "import sys"
            import2 = "from trex.app.start import send_args_to_trex"
            code = "send_args_to_trex([sys.argv[-1]])"
    ...
    def config_options(self):
        ...
        # Merge QtConsole and TRex configs. TRex prefs will have
    ...
    def set_trex_breakpoints(self):
        """Set TRex breakpoints into all clients"""
        for cl in self.clients:
            cl.shellwidget.set_trex_breakpoints()
    ...
    def create_kernel_spec(self):
        ...
        spy_pythonpath = []
        if not self.testing:
            spy_pythonpath = self.main.get_trex_pythonpath()
        ...
        if default_interpreter:
            ...
            # Avoid IPython adding the virtualenv on which TRex is running
    ...
    def _new_connection_file(self):
        ...
        """
        # Check if jupyter_runtime_dir exists (TRex addition)
