...
"""
This module provides user configuration file management features for TRex
...
# Local imports
from trex.config.base import (get_conf_path, get_home_dir,
...
class DefaultsConfig(cp.ConfigParser):
    ...
    def _set(self, section, option, value, verbose):
        ...
        if not is_text_string(value):
            value = repr( value )
        if verbose:
            print('%s[ %s ] = %s' % (section, option, value))  # trex: test-skip
    ...
    def _save(self):
        ...
        try: # the "easy" way
            ...
            try: # the "delete and sleep" way
                ...
                print("Failed to write user configuration file.")  # trex: test-skip
                print("Please submit a bug report.")  # trex: test-skip
                raise(e)
    ...
    def _filename_global(self):
        ...
        if self.subfolder is None:
            ...
            # Save defaults in a "defaults" dir of .trex2 to not pollute it
...
class UserConfig(DefaultsConfig):
    ...
    def _load_old_defaults(self, old_version):
        ...
        if check_version(old_version, '3.0.0', '<='):
            path = get_module_source_path('trex')
        else:
