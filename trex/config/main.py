...
# Local import
from trex.config.base import (CHECK_ALL, EXCLUDED_NAMES, get_home_dir,
                                SUBFOLDER, TEST)
from trex.config.fonts import BIG, MEDIUM, MONOSPACE, SANS_SERIF
from trex.config.user import UserConfig
from trex.config.utils import IMPORT_EXT
from trex.utils import codeanalysis
...
# Extensions that should be visible in TRex's file/project explorers
SHOW_EXT = ['.py', '.ipynb', '.txt', '.dat', '.pdf', '.png', '.svg']


# Extensions supported by TRex (Editor or Variable explorer)
            ...
            ('color_schemes',
             {
              'names': ['emacs', 'idle', 'monokai', 'pydev', 'scintilla',
                        'trex', 'trex/dark', 'zenburn', 'solarized/light',
                        'solarized/dark'],
              'selected': 'trex',
              # ---- Emacs ----
              'emacs/name':        "Emacs",
              ...
              'trex/name':        "TRex",
              #       Name            Color     Bold  Italic
              'trex/background':  "#ffffff",
              'trex/currentline': "#f7ecf8",
              'trex/currentcell': "#fdfdde",
              'trex/occurrence':   "#ffff99",
              'trex/ctrlclick':   "#0000ff",
              'trex/sideareas':   "#efefef",
              'trex/matched_p':   "#99ff99",
              'trex/unmatched_p': "#ff9999",
              'trex/normal':     ('#000000', False, False),
              'trex/keyword':    ('#0000ff', False, False),
              'trex/builtin':    ('#900090', False, False),
              'trex/definition': ('#000000', True, False),
              'trex/comment':    ('#adadad', False, True),
              'trex/string':     ('#00aa00', False, False),
              'trex/number':     ('#800000', False, False),
              'trex/instance':   ('#924900', False, True),
              # ---- TRex/Dark ----
              'trex/dark/name':        "TRex Dark",
              #           Name             Color     Bold  Italic
              'trex/dark/background':  "#131926",
              'trex/dark/currentline': "#2b2b43",
              'trex/dark/currentcell': "#31314e",
              'trex/dark/occurrence':   "#abab67",
              'trex/dark/ctrlclick':   "#0000ff",
              'trex/dark/sideareas':   "#282828",
              'trex/dark/matched_p':   "#009800",
              'trex/dark/unmatched_p': "#c80000",
              'trex/dark/normal':     ('#ffffff', False, False),
              'trex/dark/keyword':    ('#558eff', False, False),
              'trex/dark/builtin':    ('#aa00aa', False, False),
              'trex/dark/definition': ('#ffffff', True, False),
              'trex/dark/comment':    ('#7f7f7f', False, False),
              'trex/dark/string':     ('#11a642', False, True),
              'trex/dark/number':     ('#c80000', False, False),
              'trex/dark/instance':   ('#be5f00', False, True),
              # ---- Zenburn ----
...
try:
    CONF = UserConfig('trex', defaults=DEFAULTS, load=(not TEST),
                      version=CONF_VERSION, subfolder=SUBFOLDER, backup=True,
                      raw_mode=True)
except:
    CONF = UserConfig('trex', defaults=DEFAULTS, load=False,
                      version=CONF_VERSION, subfolder=SUBFOLDER, backup=True,
                      raw_mode=True)

# Removing old .trex.ini location:
old_location = osp.join(get_home_dir(), '.trex.ini')
