...
def create_executable():
    ...
    dist = Distribution()
    name = "trex"
    ver = trex.__version__
    try:
        imp.find_module('PyQt4')
        python_qt = 'pyqt'
    except ImportError:
        python_qt = 'pyside'
    dist.setup(name="TRex", version=ver, script="trex/trex.py",
               description="Scientific PYthon Development EnviRonment",
               target_name="%s.exe" % name, icon="%s.ico" % name,
               target_dir="%s-win32-%s-sa-%s" % (name, python_qt, ver))
    trex.add_to_distribution(dist)
    ...
    try:
        ...
        pass
    dist.includes += ['trex.scientific_startup',
                      'trex.widgets.externalshell.sitecustomize']

    #XXX: ...until we are able to distribute them (see guidata.disthelpers)
    dist.excludes += ['sphinx', 'zmq', 'IPython']

    if osp.isfile("TRexdoc.chm"):
        dist.add_data_file("TRexdoc.chm")
