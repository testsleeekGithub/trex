...
# ability to run trex-win-post-install outside of bdist_wininst installer
...
def install():
    ...
    start_menu = osp.join(get_special_folder_path('CSIDL_PROGRAMS'),
                          'TRex (Py%i.%i %i bit)' % (sys.version_info[0],
                                                       sys.version_info[1],
                                                       struct.calcsize('P')*8))
    ...
    fname = osp.join(start_menu, 'TRex (full).lnk')
    create_shortcut(python, desc, fname, '"%s"' % script, workdir,
                    osp.join(ico_dir, 'trex.ico'))
    file_created(fname)

    fname = osp.join(start_menu, 'TRex-Reset all settings.lnk')
    create_shortcut(python, 'Reset TRex settings to defaults',
    ...
    winreg.SetValueEx(winreg.CreateKey(root, KEY_C1 % ("", EWS)),
                      "", 0, winreg.REG_SZ,
                      '"%s" "%s\Scripts\trex" "%%1"' % (pythonw, sys.prefix))
    ...
    fname = osp.join(desktop_folder, 'TRex.lnk')
    desc = 'Scientific Python Development EnvironmEnt, an alternative to IDLE'
    ...
    for key in (KEY_C1 % ("", EWS), KEY_C1 % ("NoCon", EWS),
        ...
        try:
            ...
            if not is_bdist_wininst:
                print("Successfully removed TRex shortcuts from Windows "\
    ...
    if not is_bdist_wininst:
        ...
        if osp.isfile(fname):
            try:
                ...
                print("Successfully removed TRex shortcuts from your desktop.",
                      file=sys.stdout)
        # clean up startmenu
        start_menu = osp.join(get_special_folder_path('CSIDL_PROGRAMS'),
                              'TRex (Py%i.%i %i bit)' % (sys.version_info[0],
        ...
        if osp.isdir(start_menu):
            for fname in os.listdir(start_menu):
                try:
                    ...
                    print("Successfully removed TRex shortcuts from your "\
            ...
            try:
                ...
                print("Successfully removed TRex shortcut folder from your "\
                      " start menu.", file=sys.stdout)
