...
from mrex.datasets.openml import (_open_openml_url,
...
from mrex.utils.testing import assert_allclose, assert_array_equal
...
def _monkey_patch_webbased_functions(context,
    ...
    # XXX: Global variable
    if test_offline:
        context.setattr(mrex.datasets.openml, 'urlopen', _mock_urlopen)
...
def test_open_openml_url_cache(monkeypatch, gzip_response, tmpdir):
    ...
    openml_path = mrex.datasets.openml._DATA_FILE.format(data_id)
...
def test_open_openml_url_unlinks_local_path(
    ...
    monkeypatch.setattr(mrex.datasets.openml, 'urlopen', _mock_urlopen)
...
def test_fetch_openml_cache(monkeypatch, gzip_response, tmpdir):
    ...
    monkeypatch.setattr(mrex.datasets.openml, 'urlopen',
                        _mock_urlopen_raise)
...
def test_fetch_openml_with_ignored_feature(monkeypatch, gzip_response):
    ...
    dataset = mrex.datasets.fetch_openml(data_id=data_id, cache=False)
