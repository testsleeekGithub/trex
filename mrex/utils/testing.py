...
import mrex
from mrex.base import (BaseEstimator, ClassifierMixin, ClusterMixin,
                          RegressorMixin, TransformerMixin)
from mrex.utils import deprecated, IS_PYPY, _IS_32BIT
from mrex.utils._unittest_backport import TestCase
...
def all_estimators(include_meta_estimators=None,
    ...
    """Get a list of all estimators from mrex.
    ...
    for importer, modname, ispkg in pkgutil.walk_packages(
            path=path, prefix='mrex.', onerror=lambda x: None):
...
def check_skip_network():
    if int(os.environ.get('MREX_SKIP_NETWORK_TESTS', 0)):
...
def create_memmap_backed_data(data, mmap_mode='r', return_folder=False):
    ...
    return_folder
    """
    temp_folder = tempfile.mkdtemp(prefix='mrex_testing_')
...
def check_docstring_parameters(func, doc=None, ignore=None):
    ...
    if (not func_name.startswith('mrex.') or
            func_name.startswith('mrex.externals')):
        return incorrect
...
def assert_run_python_script(source_code, timeout=60):
    ...
    """
    fd, source_file = tempfile.mkstemp(suffix='_src_test_mrex.py')
    os.close(fd)
    try:
        with open(source_file, 'wb') as f:
            f.write(source_code.encode('utf-8'))
        cmd = [sys.executable, source_file]
        cwd = op.normpath(op.join(op.dirname(mrex.__file__), '..'))
        env = os.environ.copy()
        try:
