...
from mrex.utils import as_float_array, check_array, check_symmetric
from mrex.utils import check_X_y
from mrex.utils import deprecated
from mrex.utils.mocking import MockDataFrame
from mrex.utils.estimator_checks import NotAnArray
from mrex.random_projection import sparse_random_matrix
from mrex.linear_model import ARDRegression
...
from mrex.utils.validation import (
    has_fit_parameter,
    check_is_fitted,
...
def test_memmap():
    ...
    with NamedTemporaryFile(prefix='mrex-test') as tmp:
...
def test_suppress_validation():
    ...
    mrex.set_config(assume_finite=True)
    assert_all_finite(X)
    mrex.set_config(assume_finite=False)
    assert_raises(ValueError, assert_all_finite, X)
