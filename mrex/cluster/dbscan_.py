...
def dbscan(X, eps=0.5, min_samples=5, metric='minkowski', metric_params=None,
    ...
    metric : string, or callable
        ...
        the options allowed by :func:`mrex.metrics.pairwise_distances` for
    ...
    <mrex.neighbors.NearestNeighbors.radius_neighbors_graph>` with
    ``mode='distance'``, then using ``metric='precomputed'`` here.

    Another way to reduce memory and computation time is to remove
    (near-)duplicate points and use ``sample_weight`` instead.

    :func:`cluster.optics <mrex.cluster.optics>` provides a similar
...
class DBSCAN(BaseEstimator, ClusterMixin):
    ...
    Examples
    --------
    >>> from mrex.cluster import DBSCAN
    >>> import numpy as np
