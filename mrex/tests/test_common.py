"""
General tests for all estimators in mrex.
"""
...
from mrex.utils.testing import all_estimators
from mrex.utils.testing import ignore_warnings
from mrex.exceptions import ConvergenceWarning, SkipTestWarning
from mrex.utils.estimator_checks import check_estimator

import mrex
from mrex.base import RegressorMixin
from mrex.cluster.bicluster import BiclusterMixin

from mrex.discriminant_analysis import LinearDiscriminantAnalysis
from mrex.linear_model.base import LinearClassifierMixin
...
from mrex.utils.estimator_checks import (
    _safe_tags,
    _construct_instance,
...
def test_configure():
    ...
    cwd = os.getcwd()
    setup_path = os.path.abspath(os.path.join(mrex.__path__[0], '..'))
    ...
    try:
        ...
        # OpenMP, unless MREX_NO_OPENMP is set. Some users might want to run
        ...
        # temporarily set MREX_NO_OPENMP, so that this test runs smoothly.
        old_env = os.getenv('MREX_NO_OPENMP')
        os.environ['MREX_NO_OPENMP'] = "True"
        ...
        if old_env is not None:
            os.environ['MREX_NO_OPENMP'] = old_env
        else:
            del os.environ['MREX_NO_OPENMP']
        os.chdir(cwd)
...
def test_import_all_consistency():
    ...
    pkgs = pkgutil.walk_packages(path=mrex.__path__, prefix='mrex.',
                                 onerror=lambda _: None)
    submods = [modname for _, modname, _ in pkgs]
    for modname in submods + ['mrex']:
        if ".tests." in modname:
            continue
...
def test_root_import_all_completeness():
    ...
    for _, modname, _ in pkgutil.walk_packages(path=mrex.__path__,
                                               onerror=lambda _: None):
        if '.' in modname or modname.startswith('_') or modname in EXCEPTIONS:
            continue
        assert modname in mrex.__all__
...
def test_all_tests_are_importable():
    ...
    lookup = {name: ispkg
              for _, name, ispkg
              in pkgutil.walk_packages(mrex.__path__, prefix='mrex.')}
