...
# ./mrex is added to sys.path by pytest. See
# https://docs.pytest.org/en/latest/pythonpath.html for more details.  For
# example, this allows to build extensions in place and run pytest
# doc/modules/clustering.rst and use mrex from the local folder rather than
...
from mrex.utils import _IS_32BIT
...
def pytest_collection_modifyitems(config, items):
    ...
    if platform.python_implementation() == 'PyPy':
        ...
        for item in items:
            if item.name in (
                    'mrex.feature_extraction.hashing.FeatureHasher',
                    'mrex.feature_extraction.text.HashingVectorizer'):
