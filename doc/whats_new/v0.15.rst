...
- ``nose`` is non-longer a runtime dependency to import ``mrex``, only for
...
- The ``img_to_graph`` and ``grid_tograph`` functions in
  :mod:`mrex.feature_extraction.image` now return ``np.ndarray``
...
- Speed improvement of the :mod:`mrex.ensemble.gradient_boosting` module.
  By `Gilles Louppe`_ and `Peter Prettenhofer`_.

- Various enhancements to the  :mod:`mrex.ensemble.gradient_boosting`
...
- Faster :class:`mrex.ensemble.ExtraTrees` by caching feature values.
  By `Arnaud Joly`_.
...
- :mod:`mrex.hmm` is deprecated. Its removal is planned
