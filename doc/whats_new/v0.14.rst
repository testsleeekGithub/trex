...
- Added :ref:`biclustering <biclustering>` algorithms
  (:class:`mrex.cluster.bicluster.SpectralCoclustering` and
  :class:`mrex.cluster.bicluster.SpectralBiclustering`), data
  generation methods (:func:`mrex.datasets.make_biclusters` and
  :func:`mrex.datasets.make_checkerboard`), and scoring metrics
  (:func:`mrex.metrics.consensus_score`). By `Kemal Eren`_.
...
- Fixed :mod:`mrex.linear_model.stochastic_gradient.py` L2 regularization
...
- :class:`grid_search.GridSearchCV` and
  ...
  and `Lars Buitinck`_.
  Passing a function from :mod:`mrex.metrics` as ``score_func`` is
  deprecated.
...
- The new estimator :class:`mrex.decomposition.TruncatedSVD`
...
- The default number of components for
  :class:`mrex.decomposition.RandomizedPCA` is now correctly documented
  to be ``n_features``. This was the default behavior, so programs using it
  will continue to work as they did.

- :class:`mrex.cluster.KMeans` now fits several orders of magnitude
...
- Verbose output in :mod:`mrex.ensemble.gradient_boosting` now uses
  a column format and prints progress in decreasing frequency.
  It also shows the remaining time. By `Peter Prettenhofer`_.

- :mod:`mrex.ensemble.gradient_boosting` provides out-of-bag improvement
  :attr:`~mrex.ensemble.GradientBoostingRegressor.oob_improvement_`
...
- Fixed a bug in :class:`mrex.covariance.GraphLassoCV`: the
  'alphas' parameter now works as expected when given a list of
  values. By Philippe Gervais.

- Fixed an important bug in :class:`mrex.covariance.GraphLassoCV`
...
- Out-of-core learning support for discrete naive Bayes classifiers
  :class:`mrex.naive_bayes.MultinomialNB` and
  :class:`mrex.naive_bayes.BernoulliNB` by adding the ``partial_fit``
...
- Significant speed improvements for :class:`mrex.cluster.DBSCAN`
...
- Testing scikit-learn with ``mrex.test()`` is deprecated. Use
  ``nosetests mrex`` from the command line.
...
- :class:`mrex.neighbors.BallTree` has been refactored, and a
  :class:`mrex.neighbors.KDTree` has been
...
- :class:`mrex.neighbors.KernelDensity` has been added, which performs
  efficient kernel density estimation with a variety of kernels.

- :class:`mrex.decomposition.KernelPCA` now always returns output with
...
- ``gcv_mode="auto"`` no longer tries to perform SVD on a densified
  sparse matrix in :class:`mrex.linear_model.RidgeCV`.

- Sparse matrix support in :class:`mrex.decomposition.RandomizedPCA`
...
- Attribute ``oob_score_`` in :class:`mrex.ensemble.GradientBoostingRegressor`
  and :class:`mrex.ensemble.GradientBoostingClassifier`
...
- :class:`mrex.preprocessing.StandardScaler` now converts integer input
  to float, and raises a warning. Previously it rounded for dense integer
  input.

- :class:`mrex.multiclass.OneVsRestClassifier` now has a
