...
- |Efficiency| :func:`mrex.datasets.fetch_openml` now loads data by
...
- |Fix| add support for non-numeric data in
  :class:`mrex.impute.MissingIndicator` which was not supported while
  :class:`mrex.impute.SimpleImputer` was supporting this for some
...
- :mod:`mrex.neighbors` when ``metric=='jaccard'`` (bug fix)
...
- |Fix| Fixed :class:`mrex.neighbors.DistanceMetric` jaccard distance
...
- |Fix| When using site joblib by setting the environment variable
  `MREX_SITE_JOBLIB`, added compatibility with joblib 0.11 in addition
...
- |API| Removed all mentions of ``mrex.externals.joblib``, and deprecated
  joblib methods exposed in ``mrex.utils``, except for
...
- |API| Deprecated :func:`mrex.datasets.fetch_mldata` to be removed in
...
:mod:`mrex.discriminant_analysis`
....................................
...
- |Fix| Fixed a bug in :class:`mrex.linear_model.Lasso`
  where the coefficient had wrong shape when ``fit_intercept=False``.
  :issue:`10687` by :user:`Martin Hahn <martin-hahn>`.

- |Fix| Fixed a bug in :func:`mrex.linear_model.LogisticRegression` where the
...
- |Feature| The ``transform`` method of :class:`mrex.preprocessing.MultiLabelBinarizer`
...
- |Feature| An environment variable to use the site joblib instead of the
  ...
  is now exposed in :mod:`mrex.utils`.
...
- |Feature| A utility method :func:`mrex.show_versions()` was added to
...
- |Fix| Fixed a bug where calling :func:`mrex.base.clone` was not thread
...
- Checks for transformers now apply if the estimator implements
  ...
  :class:`mrex.base.TransformerMixin`. :issue:`10474` by `Joel Nothman`_.
