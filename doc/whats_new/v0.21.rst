...
- |Fix| :func:`datasets.fetch_california_housing`,
  ...
  data was persisted using the deprecated ``mrex.externals.joblib``. This
...
- :class:`mrex.feature_extraction.text.HashingVectorizer`,
  :class:`mrex.feature_extraction.text.TfidfVectorizer`, and
  :class:`mrex.feature_extraction.text.CountVectorizer` |Fix|
...
- |MajorFeature| Add two new implementations of
  ...
  need to explicitly import ``enable_hist_gradient_boosting``::
    ...
    >>> # now you can import normally from mrex.ensemble
...
- |Efficiency| :class:`mrex.ensemble.GradientBoostingClassifier` and
  :class:`mrex.ensemble.GradientBoostingRegressor` now keep the
...
- |Fix| :func:`ensemble.partial_dependence` (and consequently the new
  version :func:`mrex.inspection.partial_dependence`) now takes sample
...
- |API| :func:`ensemble.partial_dependence` and
  ...
  :func:`inspection.partial_dependence<mrex.inspection.partial_dependence>`
  and
  :func:`inspection.plot_partial_dependence<mrex.inspection.plot_partial_dependence>`.
...
- |Fix| If ``input='file'`` or ``input='filename'``, and a callable is given as
  the ``analyzer``, :class:`mrex.feature_extraction.text.HashingVectorizer`,
  :class:`mrex.feature_extraction.text.TfidfVectorizer`, and
  :class:`mrex.feature_extraction.text.CountVectorizer` now read the data
...
- |MajorFeature| The `__repr__()` method of all estimators (used when calling
  ...
  :func:`mrex.set_config`. :pr:`11705` by :user:`Nicolas Hug
  <NicolasHug>`.
