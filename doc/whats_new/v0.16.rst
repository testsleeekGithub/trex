...
- :class:`mrex.neighbors.BallTree` and :class:`mrex.neighbors.KDTree`
...
- Correct ``partial_fit`` handling of ``class_prior`` for
  ...
  :class:`mrex.naive_bayes.BernoulliNB`. By `Trevor Stephens`_.
...
- Avoid skipping the first nearest neighbor in the methods ``radius_neighbors``,
  ...
  :class:`mrex.neighbors.NearestNeighbors` and family, when the query
...
- Users should now supply an explicit ``average`` parameter to
  :func:`mrex.metrics.f1_score`, :func:`mrex.metrics.fbeta_score`,
  :func:`mrex.metrics.recall_score` and
  :func:`mrex.metrics.precision_score` when performing multiclass
...
- Allow ``X=None`` in the methods ``radius_neighbors``, ``kneighbors``,
  ...
  :class:`mrex.neighbors.NearestNeighbors` and family. If set to None,
