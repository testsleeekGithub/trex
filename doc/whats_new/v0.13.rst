...
- Improve narrative documentation and consistency in
  :mod:`mrex.metrics` for regression and classification metrics
  by `Arnaud Joly`_.

- Fixed a bug in :class:`mrex.svm.SVC` when using csr-matrices with
...
- The module ``mrex.linear_model.sparse`` is gone. Sparse matrix support
  was already integrated into the "regular" linear models.

- :func:`mrex.metrics.mean_square_error`, which incorrectly returned the
...
- Renamed function :func:`mrex.metrics.zero_one` to
  :func:`mrex.metrics.zero_one_loss`. Be aware that the default behavior
  in :func:`mrex.metrics.zero_one_loss` is different from
  :func:`mrex.metrics.zero_one`: ``normalize=False`` is changed to
  ``normalize=True``.
