...
on your hard-drive named ``mrex_tut_workspace`` where you
will edit your own files for the exercises while keeping
the original skeletons intact::

    % cp -r skeletons work_directory/mrex_tut_workspace
...
manually from the website and use the :func:`mrex.datasets.load_files`
...
to work with, ``scikit-learn`` provides a :class:`~mrex.pipeline.Pipeline` class that behaves
...
classifier object into our pipeline::
  ...
  >>> from mrex.linear_model import SGDClassifier
  >>> text_clf = Pipeline([
  ...     ('vect', CountVectorizer()),
  ...     ('tfidf', TfidfTransformer()),
...
.. note:
  ...
  Alternatively we could have used ``mrex.svm.LinearSVC`` (Linear
