...
.. currentmodule:: mrex.gaussian_process
...
.. currentmodule:: mrex.gaussian_process.kernels
...
kernel but with the hyperparameters set to ``theta``. An illustrative example:
    ...
    >>> from mrex.gaussian_process.kernels import ConstantKernel, RBF
...
All Gaussian process kernels are interoperable with :mod:`mrex.metrics.pairwise`
and vice versa: instances of subclasses of :class:`Kernel` can be passed as
``metric`` to ``pairwise_kernels`` from :mod:`mrex.metrics.pairwise`. Moreover,
