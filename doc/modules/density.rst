...
Gaussian Mixtures (:class:`mrex.mixture.GaussianMixture`), and
neighbor-based approaches such as the kernel density estimate
(:class:`mrex.neighbors.KernelDensity`).
...
:class:`mrex.neighbors.KernelDensity` estimator, which uses the
...
follows:
   ...
   >>> from mrex.neighbors.kde import KernelDensity
   >>> import numpy as np
...
:class:`mrex.neighbors.KernelDensity` implements several common kernel
...
metrics (see :class:`mrex.neighbors.DistanceMetric` for a list of available metrics), though
