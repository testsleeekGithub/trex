...
.. currentmodule:: mrex.preprocessing

The ``mrex.preprocessing`` package provides several common
...
operation on a single array-like dataset::
  ...
  >>> from mrex import preprocessing
  >>> import numpy as np
...
.. topic:: Scaling vs Whitening
  ...
  To address this issue you can use :class:`mrex.decomposition.PCA` with
...
regression (:class:`LinearRegression <mrex.linear_model.LinearRegression>`),
...
in a :class:`mrex.pipeline.Pipeline`.
...
this is the case for the :class:`mrex.neural_network.BernoulliRBM`.
...
:class:`mrex.pipeline.Pipeline`. The ``fit`` method does nothing
...
Often it's useful to add complexity to the model by considering nonlinear features of the input data. A simple and common method to use is polynomial features, which can get features' high-order and interaction terms. It is implemented in :class:`PolynomialFeatures`::
    ...
    >>> import numpy as np
    >>> from mrex.preprocessing import PolynomialFeatures
...
Note that polynomial features are used implicitly in `kernel methods <https://en.wikipedia.org/wiki/Kernel_method>`_ (e.g., :class:`mrex.svm.SVC`, :class:`mrex.decomposition.KernelPCA`) when using polynomial :ref:`svm_kernels`.
