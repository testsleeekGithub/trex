...
:class:`mrex.feature_extraction.FeatureHasher` for datasets with categorical
variables represented as list of Python dicts or
:class:`mrex.feature_extraction.text.HashingVectorizer` for text documents.
...
Here is a list of incremental estimators for different tasks:
  ...
  - Classification
      + :class:`mrex.naive_bayes.MultinomialNB`
  ...
  - Regression
      + :class:`mrex.linear_model.SGDRegressor`
      + :class:`mrex.linear_model.PassiveAggressiveRegressor`
      + :class:`mrex.neural_network.MLPRegressor`
  - Clustering
      + :class:`mrex.cluster.MiniBatchKMeans`
      + :class:`mrex.cluster.Birch`
  - Decomposition / feature Extraction
      + :class:`mrex.decomposition.MiniBatchDictionaryLearning`
      + :class:`mrex.decomposition.IncrementalPCA`
      + :class:`mrex.decomposition.LatentDirichletAllocation`
  - Preprocessing
      + :class:`mrex.preprocessing.StandardScaler`
      + :class:`mrex.preprocessing.MinMaxScaler`
      + :class:`mrex.preprocessing.MaxAbsScaler`
...
``MREX_ASSUME_FINITE`` to a non-empty string before importing
scikit-learn, or configure it in Python with :func:`mrex.set_config`.
...
allows you to set this configuration within a specified context::
  ...
  >>> import mrex
  >>> with mrex.config_context(assume_finite=True):
  ...     pass  # do learning/prediction here with reduced validation

Note that this will affect all uses of
:func:`mrex.utils.assert_all_finite` within the context.
...
For :mod:`mrex.linear_model` (e.g. Lasso, ElasticNet,
...
:class:`mrex.linear_model.stochastic_gradient.SGDClassifier` with the
...
For the :mod:`mrex.svm` family of algorithms with a non-linear kernel,
...
:class:`mrex.svm.classes.NuSVR` was used to influence the number of
support vectors.
...
For :mod:`mrex.ensemble` of trees (e.g. RandomForest, GBT,
...
:class:`mrex.ensemble.gradient_boosting.GradientBoostingRegressor`.
...
working memory (defaulting to 1GB) using :func:`mrex.set_config` or
:func:`config_context`.  The following suggests to limit temporary working
memory to 128 MiB::

  >>> import mrex
  >>> with mrex.config_context(working_memory=128):
...
:func:`mrex.set_config` controls the following behaviors:
