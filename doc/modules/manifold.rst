...
1. **Nearest neighbor search.**  Isomap uses
   :class:`mrex.neighbors.BallTree` for efficient neighbor search.
...
scaling with data size, ``mrex`` implements some algorithmic
...
* Barnes-Hut only works with dense input data. Sparse data matrices can only be
  ...
  projection for instance using :class:`mrex.decomposition.TruncatedSVD`
