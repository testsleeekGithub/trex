...
unlabeled data can be performed with the module :mod:`mrex.cluster`.
...
.. topic:: Input data
    ...
    These can be obtained from the classes in the :mod:`mrex.feature_extraction`
    ...
    in the :mod:`mrex.metrics.pairwise` module.
...
graph, and SpectralClustering is initialized with `affinity='precomputed'`::
    ...
    >>> from mrex.cluster import SpectralClustering
...
.. warning:: **Connectivity constraints with single, average and complete linkage**
    ...
    :func:`mrex.neighbors.kneighbors_graph`. In the limit of a small
...
.. topic:: Memory consumption for large sample sizes
    ...
    - A sparse radius neighborhood graph (where missing entries are presumed to
      ...
      :meth:`mrex.neighbors.NearestNeighbors.radius_neighbors_graph`.
...
The Fowlkes-Mallows index (:func:`mrex.metrics.fowlkes_mallows_score`) can be
...
(:func:`mrex.metrics.calinski_harabasz_score`) - also known as the Variance
...
(:func:`mrex.metrics.davies_bouldin_score`) can be used to evaluate the
...
cluster analysis as follows:
  ...
  >>> X = iris.data
  >>> from mrex.cluster import KMeans
  >>> from mrex.metrics import davies_bouldin_score
...
Contingency matrix (:func:`mrex.metrics.cluster.contingency_matrix`)
...
Here is an example::
   ...
   >>> from mrex.metrics.cluster import contingency_matrix
   >>> x = ["a", "a", "a", "b", "b", "b"]
   >>> y = [0, 0, 1, 1, 2, 2]
