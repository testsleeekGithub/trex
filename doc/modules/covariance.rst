...
`mrex.covariance` package provides tools for accurately estimating
...
`mrex.covariance` package, or it can be otherwise obtained by
...
with the :meth:`oas` function of the `mrex.covariance`
...
The ``mrex.covariance`` package implements a robust estimator of covariance,
