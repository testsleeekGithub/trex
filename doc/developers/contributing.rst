...
- Please include your **operating system type and version number**, as well as
  ...
  can be found by running the following code snippet::
    ...
    >>> import mrex
    >>> mrex.show_versions()  # doctest: +SKIP
  ...
  .. note::
    ...
    For previous versions, one has to explicitly run::
     ...
     import mrex; print("Scikit-Learn", mrex.__version__)
...
2. **Make sure your code passes the tests**. The whole test suite can be run
   ...
   `mrex/linear_model/logistic.py`, running the following commands will
   usually be enough:

   - `pytest mrex/linear_model/logistic.py` to make sure the doctest
     examples are correct
   - `pytest mrex/linear_model/tests/test_logistic.py` to run the tests
     specific to the file
   - `pytest mrex/linear_model` to test the whole `linear_model` module
   - `pytest mrex/doc/linear_model.rst` to make sure the user guide
     examples are correct.
   - `pytest mrex/tests/test_common.py -k LogisticRegression` to run all our
...
7. When applicable, use the validation tools and scripts in the
   ``mrex.utils`` submodule.  A list of utility routines available
...
* Code with a good unittest coverage (at least 80%, better 100%), check
  with::
    ...
    $ pytest --cov mrex path/to/tests_for_package
...
* For unwritten formatting rules, try to follow existing good works:
    ...
    * For "References" in docstrings, see the Silhouette Coefficient
      (:func:`mrex.metrics.silhouette_score`).
...
we add the decorator ``deprecated`` (from ``mrex.utils``)
...
- The trickiest thing is often to identify which portions of the code are
  ...
  :class:`mrex.linear_model.LinearRegression`, what you're looking for
...
- Due to the use of `Inheritance
  ...
  at least from :class:`BaseEstimator <mrex.base.BaseEstimator>`, and
  from a ``Mixin`` class (e.g. :class:`ClassifierMixin
  <mrex.base.ClassifierMixin>`) that enables default behaviour depending
