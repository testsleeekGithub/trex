...
:func:`utils.estimator_checks.check_estimator` on the class::
  ...
  >>> from mrex.utils.estimator_checks import check_estimator
...
:func:`~mrex.utils.parametrize_with_checks`, making it easier to test
multiple estimators::
  ...
  def test_mrex_compatible_estimator(estimator, check):
      check(estimator)
...
.. topic:: ``BaseEstimator`` and mixins:
    ...
    and optionally the mixin classes in ``mrex.base``.
    ...
    `project template <https://github.com/scikit-learn-contrib/project-template/blob/master/skltemplate/_template.py>`__.
      ...
      >>> from mrex.utils.validation import check_X_y, check_array, check_is_fitted
      >>> from mrex.utils.multiclass import unique_labels
      >>> from mrex.metrics import euclidean_distances
...
``__repr__`` method, is to inherit from ``mrex.base.BaseEstimator``. If you
...
independent term is stored in ``intercept_``.  ``mrex.linear_model.base``
contains a few base classes and mixins that implement common linear model
patterns.

The :mod:`mrex.utils.multiclass` module contains useful functions
...
tags are used by the common tests and the :func:`mrex.utils.estimator_checks.check_estimator` function to
...
poor_score
    ...
    are based on current estimators in mrex and might be replaced by
...
* Unit tests are an exception to the previous rule;
  ...
  A corollary is that, if ``mrex.foo`` exports a class or function
  that is implemented in ``mrex.foo.bar.baz``,
  the test should import it from ``mrex.foo``.
...
.. currentmodule:: mrex.utils

The module :mod:`mrex.utils` contains various functions for doing input
...
See :func:`mrex.utils.check_random_state` in :ref:`developers-utils`.

Here's a simple example of code using some of the above guidelines::

    from mrex.utils import check_array, check_random_state
