...
request with 0.999.X as base. Add a commit updating ``mrex.__version__``.
...
2. On the branch for releasing, update the version number in
   mrex/__init__.py, the ``__version__`` variable by removing ``dev*`` only
...
The :mod:`mrex.experimental` module was introduced in 0.21 and contains
...
<https://github.com/scikit-learn/scikit-learn/blob/master/mrex/experimental/enable_hist_gradient_boosting.py>`_,
or
`enable_iterative_imputer.py
<https://github.com/scikit-learn/scikit-learn/blob/master/mrex/experimental/enable_iterative_imputer.py>`_.

Note that the public import path must be to a public subpackage (like
``mrex/ensemble`` or ``mrex/impute``), not just a ``.py`` module.
Also, the (private) experimental features that are imported must be in a
submodule/subpackage of the public subpackage, e.g.
``mrex/ensemble/_hist_gradient_boosting/`` or
``mrex/impute/_iterative.py``. This is needed so that pickles still work
in the future when the features aren't experimental anymore

Please also write basic tests following those in
`test_enable_hist_gradient_boosting.py
<https://github.com/scikit-learn/scikit-learn/blob/master/mrex/experimental/tests/test_enable_hist_gradient_boosting.py>`_.
...
mrex.experimental import *`` **does not work**.

Note that some experimental classes / functions are not included in the
:mod:`mrex.experimental` module: ``mrex.datasets.fetch_openml``.
