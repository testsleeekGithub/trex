...
located in :mod:`mrex.utils`, and include tools in a number of categories.
All the following functions and classes are in the module :mod:`mrex.utils`.
...
- :func:`validation.check_memory` checks that input is ``joblib.Memory``-like,
  ...
  ``mrex.utils.Memory`` instance (typically a str denoting
...
- :func:`arrayfuncs.cholesky_delete`:
  (used in :func:`mrex.linear_model.lars_path`)  Remove an
  item from a cholesky factorization.

- :func:`arrayfuncs.min_pos`: (used in ``mrex.linear_model.least_angle``)
...
- :func:`shuffle`: Shuffle arrays or sparse matrices in a consistent way.
  Used in :func:`mrex.cluster.k_means`.
...
The ``mrex.utils.sparsefuncs`` cython module hosts compiled extensions to
...
- :func:`sparsefuncs_fast.inplace_csr_row_normalize_l1` and
  ...
  :class:`mrex.preprocessing.Normalizer`.
...
- :func:`graph_shortest_path.graph_shortest_path`:
  (used in :class:`mrex.manifold.Isomap`)
...
- :class:`gen_even_slices`: generator to create ``n``-packs of slices going up
  to ``n``.  Used in :func:`mrex.decomposition.dict_learning` and
  :func:`mrex.cluster.k_means`.
...
- :func:`murmurhash3_32` provides a python wrapper for the
  ...
  random projections::
    ...
    >>> from mrex.utils import murmurhash3_32
  ...
  The ``mrex.utils.murmurhash`` module can also be "cimported" from
...
- :class:`mrex.exceptions.ConvergenceWarning`: Custom warning to catch
  convergence problems. Used in ``mrex.covariance.graphical_lasso``.
